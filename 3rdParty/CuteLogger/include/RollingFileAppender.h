...
class CUTELOGGERSHARED_EXPORT RollingFileAppender : public FileAppender {
   public:
    ...
    void setLogFilesMaxSize(int maximum);
    qint64 logFilesMaxSize() const;

   protected:
    virtual void append(const QDateTime& timeStamp, Logger::LogLevel logLevel, const char* file, int line, const char* function, const QString& category,
                        const QString& message);

   private:
    void rollOver(bool force = false);
    ...
    qint64 m_logFilesMaxSize;
    QElapsedTimer m_sizeCheckTimer;
};
