...
RollingFileAppender::RollingFileAppender(const QString& fileName)
    : FileAppender(fileName), m_frequency(DailyRollover), m_logFilesLimit(0), m_logFilesMaxSize(0) {
    //    m_sizeCheckTimer.start();
}

void RollingFileAppender::append(const QDateTime& timeStamp, Logger::LogLevel logLevel, const char* file, int line, const char* function,
                                 const QString& category, const QString& message) {
    if (!m_rollOverTime.isNull() && QDateTime::currentDateTime() > m_rollOverTime) rollOver();
    if (m_logFilesMaxSize && m_sizeCheckTimer.hasExpired(60 * 1000)) {
        m_sizeCheckTimer.restart();
        QFileInfo logFileInfo(fileName());
        if (logFileInfo.size() > m_logFilesMaxSize) {
            rollOver(true);
        }
    }
...
void RollingFileAppender::setDatePattern(DatePattern datePattern) {
    switch (datePattern) {
        ...
        case MonthlyRollover:
            setDatePatternString(QLatin1String("'_'yyyy-MM-dd_hh-mm-ss"));
            break;
...
void RollingFileAppender::removeOldFiles() {
    ...
    logDirectory.setNameFilters(QStringList() << fileInfo.baseName() + "_*");
    QFileInfoList logFiles = logDirectory.entryInfoList();
    QMap<QDateTime, QString> fileDates;
    for (int i = 0; i < logFiles.length(); ++i) {
        QString name = logFiles[i].baseName();
        QString suffix = name.mid(name.indexOf(fileInfo.baseName()) + fileInfo.baseName().length());
...
void RollingFileAppender::computeRollOverTime() {
    ...
    Q_ASSERT_X(m_rollOverSuffix != m_rollOverTime.toString(m_datePatternString), "DailyRollingFileAppender::computeRollOverTime()",
               "File name does not change with rollover");
}

void RollingFileAppender::rollOver(bool force) {
    ...
    if (force) {
        rollOverSuffix = QDateTime::currentDateTime().toString(m_datePatternString);
    } else {
        computeRollOverTime();
    ...
    QString targetFileName = original.canonicalPath() + QDir::separator() + original.baseName() + rollOverSuffix;
    if (!original.completeSuffix().isEmpty()) targetFileName += "." + original.completeSuffix();
...
void RollingFileAppender::setLogFilesMaxSize(int maximum) {
    QMutexLocker locker(&m_rollingMutex);
    m_logFilesMaxSize = maximum;
}
int RollingFileAppender::logFilesLimit() const {
    QMutexLocker locker(&m_rollingMutex);
    return m_logFilesLimit;
}
qint64 RollingFileAppender::logFilesMaxSize() const {
    QMutexLocker locker(&m_rollingMutex);
    return m_logFilesMaxSize;
}
