...
#include "ui_cmu4u.h"
cmu4u::cmu4u(QWidget* parent) : QWidget(parent), ui(new Ui::cmu4u) {
    ui->setupUi(this);
    QList<QWidget*> blockTabs;
    blockTabs << ui->tabSOE << ui->tabExt;
    foreach (QWidget* w, blockTabs) { ui->DataWidget->removeTab(ui->DataWidget->indexOf(w)); }
    //    this->installEventFilter(this);
    this->timer = new QTimer(this);
    connect(timer, &QTimer::timeout, this, &cmu4u::timerUpDate);
    mycmu = nullptr;
    ...
    connect(ui->connectIP, &QLineEdit::editingFinished, this, &cmu4u::IpChange, Qt::UniqueConnection);
    ...
    foreach (QDoubleSpinBox* dspbox, dspboxs) {
        connect(dspbox, static_cast<void (QDoubleSpinBox::*)(double)>(&QDoubleSpinBox::valueChanged), this,
                &cmu4u::valueChange, Qt::UniqueConnection);
    }
    ...
    foreach (QPushButton* btn, btns) {
        connect(btn, &QPushButton::released, this, &cmu4u::btn_released, Qt::UniqueConnection);
    }
    //    QList<QCheckBox*> chkboxs = ui->G_FuncMask->findChildren<QCheckBox*>();
    //    foreach (QCheckBox* chkbox, chkboxs) {
    //        connect(chkbox, &QCheckBox::stateChanged, this, &cmu4u::stateChanged, Qt::UniqueConnection);
    //    }
    /*    QList<QCheckBox*> RadioList;
        ...
        foreach (QCheckBox* rb, RadioList) {
            connect(rb, &QCheckBox::stateChanged, this, &cmu4u::checkChanged, Qt::UniqueConnection);
        }*/  //
    connect(ui->btnReadSOE, &QPushButton::released, this, &cmu4u::btn_contrl, Qt::UniqueConnection);
...
void cmu4u::uiInit() {
    ...
    if (this->mycmu->GetProtocalVer() > CMUV3) {
        ui->DataWidget->setTabEnabled(ui->DataWidget->indexOf(ui->tabExt), true);
        ...
        ui->DataWidget->setTabEnabled(ui->DataWidget->indexOf(ui->tabExt), false);
    }
    QList<QLabel*> hideList;
    hideList << ui->bSysInit << ui->bSysBalance << ui->bSysCharge << ui->bSysDischarge << ui->bSysStop << ui->bSys10
             << ui->bSys11 << ui->bSys12 << ui->bSys13 << ui->bSys14 << ui->bSys15;
    hideList << ui->bDI3 << ui->bDI4 << ui->bDI5 << ui->bDI6 << ui->bDI7 << ui->bDI8 << ui->bDI9 << ui->bDI10
             << ui->bDI11 << ui->bDI12 << ui->bDI13 << ui->bDI14 << ui->bDI15;
    hideList << ui->bErr4 << ui->bErr5 << ui->bErr11 << ui->bErr12 << ui->bErr14 << ui->bErr15;
    hideList << ui->bAlm4 << ui->bAlm5 << ui->bAlm8 << ui->bAlm11 << ui->bAlm12 << ui->bAlm14 << ui->bAlm15;
    for (int i = 0; i < hideList.size(); i++) {
        hideList.at(i)->hide();
    }
}
int cmu4u::setValue(string name, double dval) {
    map<string, NodeReg>::iterator iter1;
...
void cmu4u::flushData() {
    ...
    //        QList<QCheckBox*> RadioList;
    //        RadioList << ui->bDO0 << ui->bDO1 << ui->bDO2 << ui->bDO3 << ui->bDO4 << ui->bDO5 << ui->bDO6 << ui->bDO7
    //                  << ui->bDO8 << ui->bDO9 << ui->bDO10 << ui->bDO11 << ui->bDO12 << ui->bDO13 << ui->bDO14 <<
    //                  ui->bDO15;
    //        foreach (QCheckBox* rb, RadioList) {
    //            try {
    ...
    //        CheckBoxList << ui->bFunc0 << ui->bFunc1 << ui->bFunc2 << ui->bFunc3 << ui->bFunc4 << ui->bFunc5 <<
    //        ui->bFunc6
    ...
    if (ui->DataWidget->currentWidget()->objectName() == tr("tabExt")) {
        QTableWidgetItem* item;
        int offset = 0;
        for (int i = 0; i < config.bmu_num; i++) {
            offset = 0;
...
void cmu4u::on_lineEditIP_editingFinished() {
...
void cmu4u::on_lineEditServIP_editingFinished() {
...
void cmu4u::on_btnOutput_released() {
...
void cmu4u::on_btnInput_released() {
...
//屏蔽本控件传递事件到父控件
bool cmu4u::eventFilter(QObject* obj, QEvent* event) {
    Q_UNUSED(obj);
    Q_UNUSED(event);
    if (obj == ui->tableBMU) {
        qDebug() << obj << event;
...
void cmu4u::on_cbProtocol_currentIndexChanged(const QString& arg1) {
    qDebug() << arg1;
...
void cmu4u::initUpdateMenu() {
    update_menu = new QMenu;
    update_menu->addAction("下载升级BMS", this, &cmu4u::onUpdateBtnMenu);
    update_menu->addAction("下载升级BMU", this, &cmu4u::onUpdateBtnMenu);
    update_menu->addAction("下载升级BMS Boot", this, &cmu4u::onUpdateBtnMenu);
    update_menu->addAction("下载升级BMU Boot", this, &cmu4u::onUpdateBtnMenu);
    update_menu->addAction("下载升级绝缘板", this, &cmu4u::onUpdateBtnMenu);
    update_menu->addAction("升级BMU", this, &cmu4u::onUpdateBtnMenu);
    ui->btnVer->setMenu(update_menu);
}

void cmu4u::onUpdateBtnMenu() {
...
void cmu4u::on_checkBox_stateChanged(int arg1) {
