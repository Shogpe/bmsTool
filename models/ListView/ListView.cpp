    ...
    switch (eventType) {
        ...
        case QEvent::HoverMove: {
            ...
            QMetaObject::invokeMethod(qobject_cast<StyledDelegate*>(itemDelegate(modelIndex)), "hoverEvent",
                                      Q_ARG(QHoverEvent*, pHoverEvent), Q_ARG(QAbstractItemView*, this),
                                      Q_ARG(QStyleOptionViewItem, option), Q_ARG(QModelIndex, modelIndex));
        } break;
        ...
        case QEvent::MouseButtonDblClick: {
            ...
            QMetaObject::invokeMethod(qobject_cast<StyledDelegate*>(itemDelegate(modelIndex)), "mouseEvent",
                                      Q_RETURN_ARG(int, role), Q_ARG(QMouseEvent*, mouseEvent),
                                      Q_ARG(QAbstractItemView*, this), Q_ARG(QStyleOptionViewItem, option),
                                      Q_ARG(QModelIndex, modelIndex));

            if (eventType == QEvent::MouseButtonRelease && Qt::LeftButton == mouseEvent->button()) {
                // 左键按下
                if (role != -1) {
            ...
            if (eventType == QEvent::MouseButtonDblClick && Qt::LeftButton == mouseEvent->button()) {
                // 左键双击
