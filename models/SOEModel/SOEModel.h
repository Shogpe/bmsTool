...
class SOEModel : public QAbstractTableModel {
   ...
   private:
    const SOE_MAP soe_map {
        {1, {UINT16, 0.0001, "单体电池低压告警", "最小电压", "门限电压"}},
    ...
    QVariant data(const QModelIndex& index, int role) const override {
        ...
        if (role == Qt::TextAlignmentRole) {
            ...
            switch (index.column()) {
                ...
                case 2: {
                    SOE_MAP::const_iterator SOEIter;
                    SOEIter = soe_map.find(index_soe.soe_type);
                    if (SOEIter != soe_map.end()) {
                        double val = 0, val_limit = 0;
                        if (SOEIter->val_type == UINT16) {
                            val = *(uint16_t*)&index_soe.soe_val * SOEIter->val_factor;
                            val_limit = *(uint16_t*)&index_soe.soe_limit * SOEIter->val_factor;
                        } else {
