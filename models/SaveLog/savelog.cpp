...
void SaveLog::redirectionToFile(const QString &content)
{
    QDir dir(path);
    if (!dir.exists()) {
        dir.mkdir(path);
...
void SaveLog::save(const QString &content)
{
    ...
    switch (m_direction)
    {
        case RedirectionType::Cancel :
        {
            break;
        }
        case RedirectionType::To_Form :
        {
            emit sendToForm(content);
            break;
        }
        case RedirectionType::To_Net :
        {
            emit send(content);
            break;
        }
        case RedirectionType::To_File :
        {
            redirectionToFile(content);
            break;
        }
        default:break;
    }
...
void SaveLog::setDirection(RedirectionType arg)
{

    if(arg == RedirectionType::To_File || arg == RedirectionType::To_Form)
    {
        this->start();
    }
    else if(arg == RedirectionType::To_Net)
    {
        SendLog::Instance()->start();
    }else if(arg == RedirectionType::Cancel)
    {
        this->stop();
