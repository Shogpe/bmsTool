...
/*  ----------------------------------------------------------------------------*\
    NAME: POLYNOMIAL 0xEDB88320,which is 0x04C11DB7 reversed
...
//多项式0xEDB88320,12 BE 36 B7
unsigned long crc32_buf(unsigned long inCrc32, const void *buf, size_t bufLen) {
    unsigned long crc32;
...
int crc32_file(FILE *file, unsigned long *outCrc32) {
    ...
    *outCrc32 = 0;
    while(1) {
        ...
        *outCrc32 = crc32_buf(*outCrc32, buf, bufLen);
    }
    return 0;
}
//计算结果与STM32硬件算法一致
uint32_t stm32Table[ 256 ] = {
...
//        printf("%u-%u,",nReg,pData[ n ]);
        for(i = 0; i < 4; i++) {
            nTemp = stm32Table[(uint8_t)((nReg >> 24) & 0xff) ];        //取一个字节，查表
//            printf("%u->",(uint8_t)((nReg >> 24) & 0xff));
